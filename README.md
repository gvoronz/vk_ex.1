...
6. Ваше приложение теперь будет доступно по адресу `http://localhost:3000` и будет отображать результаты пинга с сервера.

---

Теперь у вас полностью настроена система для получения и отображения данных о пинге контейнеров.
